...
## 범위
* 범위
    * 0부터 시작하는 연속적인 정수들의 모음
    * range( 생성할 연속적인 정수의 개수 )
    * range( 시작정수, 끝 비포함 정수 )
    * range( 시작정수, 끝 비포함 정수, 새로운 증가치)
    * 참고) 새로운 증가치가 음수인 경우
    ```py
    r = range(3, 0, -1)
    print(list(r)) #[3,2,1]
...
* 집합
    ...
    * 집합생성
        * 중괄호{} 사용시 자동으로 생성
        ```py
        st = {'d','a','t','a'}
        print(st) #{'d','a','t'} # 집합 생성시 자동으로 중복제거 됨
        ```
    * 추가 수정 삭제
        * 값 추가
        ```py
...
## 사전
* 사전
    * 키(단어)와 값(단어 설명)쌍의 모음
    * 사전생성
        * 중괄호{} 사용시 자동 생성
        ```py
        d = {'k1':'d','k2':'a'}
        print(d) #{'k1':'d','k2':'a'}
        print(len(d))#2 # 키와 값의 쌍 개수
        print(d['k1'])#d
        #참고)
        print(d.get('k1')) #d
        ```
    * 키 리스트
    ```py
    d = {'k1':'d','k2':'a'}
    print(d.keys())#dick_keys(['k1','k2'])
    ```
    * 값 리스트
    ```py
    d = {'k1':'d','k2':'a'}
    print(d.values())#dick_values(['d','a'])
    ```
    * 키와 값 리스트
    ```py
    d = {'k1':'d','k2':'a'}
    print(d.items())#dick_itmes([('k1', 'd'), ('k2', 'a')])
    ```   
    * 추가 수정 삭제
        * 값 추가 : 키로 값 추가
        ```py
        ...
        * 모든 값 삭제
        ...
        ```
    * 디폴트 사전
        * 없는 키로 값에 접근시 예외가 발생하는 것이 아니라 디폴트 값을 값으로 가지는 사전
        ```py
        import collections
        d = collections.defaultdict(lambda : '', {'k1':'d','k2':'a'})
        print(d['k3']) # '' 빈값 생성 예외발생X
        ```
## 컬렉션 축약
* 컬렉션 축약
    * 컬렉션 생성 후 추가하는 것이 아닌 컬렉션 (리스트. 집합, 사전) 생성시 값 추가하기
* 리스트 축약
    * 리스트 생성시 값 추가
    * 변수명 = [새로운 값 for 값 in 다른 컬렉션]
    ```py
    l = [i * i for i in range(4)]
    print(l) #['0','1','4','9']
    ```  
    * 변수명 = [새로운 값 for 값 in 다른 컬렉션 if 조건식]
    ```py
    l = [i * i for i in range(4) if i % 2 == 0]#짝수만
    ```   
* 집합 축약
    * 변수명 = {새로운 값 for 값  in 다른 컬렉션}
    * 변수명 = {새로운 값 for 값  in 다른 컬렉션 if 조건식}
* 사전 축약
    * 변수명 {새로운 키 : 새로운 값 for 값 in 다른 컬렉션}
    * 변수명 {새로운 키 : 새로운 값 for 값 in 다른 컬렉션 if 조건식}

## 조건식에서 컬렉션 데이터 타입 사용
* 조건식에서 컬렉션 데이터 타입 사용
    * 내용물이 있으면 True, 내용물이 없으면 False, 모든 컬렉션(리스트, 튜플, 범위, 집합, 사전)에 동일하게 적용됨
## 랜덤(리스트)
 |함수|정의|
 |----------------------|---------------------|
 |random.choice(변수명)| 랜덤으로 고르기 ( 한개)|
 |random.sample(변수명, 고를 갯수)| 랜덤으로 고르기 ( 여러개 )|
 |random.shuffle(변수명)| 랜덤으로 섞기|
